...
app.listen(port, () => {
    ...
    console.log(`WebSocket server listening at ws://localhost:6060`)
});
var webSockets = {}
...
wss.on('connection', function (ws, req) {
    ...
    ws.on('message', message => { // if there is any message
        ...
        if (datastring.charAt(0) == "{") {
            ...
            if (data.auth == "a65bc41dab7b9789c8dd03c486809c41") {
                if (data.cmd == 'send') {
                    var boardws = webSockets[data.reciever] // check if there is reciever connection
                    if (boardws) {
                        var cdata = "{'cmd':'" + data.cmd + "','reciever':'" + data.reciever + "','userid':'" + data.userid + "','msgtext':'" + data.msgtext + "'}";
